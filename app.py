...
def create_or_update_user(uid, username=None, first_name=None, referrer_id=None):
    ...
    if not exist:
        ...
        if referrer_id and referrer_id != uid and referrer_id != 0:
            ...
            except Exception as e: logger.error(f"Referral: {e}")
    else:
...
LANG_MAP = {
    ...
    "it":"Italiano","ar":"العربية","zh-CN":"中文","ja":"日本語","ru":"Русский",
    "tr":"Türkçe","nl":"Nederlands","pt":"Português","sv":"Svenska","no":"Norsk",
    "da":"Dansk","fi":"Suomi","pl":"Polski","cs":"Čeština","ro":"Română",
    "bg":"Български","uk":"Українська","ko":"한국어","hi":"हिन्दी","vi":"Tiếng Việt",
    "th":"ไทย","id":"Bahasa Indonesia","iw":"עברית"
}
def get_lang_name(code): return LANG_MAP.get(code, code)
...
def register():
    ...
    u = get_user(uid)
    return jsonify({"user_id":u['user_id'],"points":u['points'],"referral_link":f"https://t.me/{OFFICIAL_BOT_USERNAME}?start=ref_{uid}"})
...
def analyze():
    try:
        ...
        if user['points'] < ANALYSIS_COST:
            return jsonify({"success":False, "error":"Insufficient points"}), 402
        ...
        response = gemini_model.generate_content(
            [processed, prompt],
            generation_config=genai.types.GenerationConfig(temperature=0.9, max_output_tokens=300)
        )
        ...
        return jsonify({"success":True, "symbols":result, "language":user_lang})
    except Exception as e:
...
async def start_command(update, context):
    ...
    text = "☕ *Καλωσόρισες στο Omen!* Σου κάναμε δώρο 15 πόντους."
    if ref: text += "\n🎁 Μπήκες μέσω πρόσκλησης!"
